  ...
  <section class="order-main">
        ...
        >
          ...
          <el-form-item label="业务类型：" prop="typeId">
            <el-select
              ...
              placeholder="请选择"
              style="width:100px"
              v-if="setting" @change="submitSearch"
            >
              <el-option
                v-for="typeItem in setting.type"
                :key="typeItem.id"
                :label="typeItem.name"
                :value="typeItem.id"
              />
          ...
          <el-form-item :label="searchForm.typeId==1?'供应商：':'客户：'" prop="crmId" v-if="setting">
            <el-select v-model="searchForm.crmId" placeholder="请选择" clearable filterable>
              <el-option
                v-for="crm in setting.crm.filter(o=>o.typeId==searchForm.typeId)"
                :key="crm.id"
                :label="crm.name"
                :value="crm.id"
              />
        ...
        <el-table-column label="操作" fixed="right" align="center" width="70">
          <template slot-scope="scope">
            <div>
              ...
              >{{scope.row.isAdded?'查阅':'入库'}}</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="仓库入库清单" append-to-body :visible.sync="openOrderVisible" width="85%">
      <div class="order-title">
        <span
          v-if="currItem"
        >客户：{{currItem.crmName}}，地址：{{currItem.address}}，联系人：{{currItem.contactName}}，电话：{{currItem.contactPhone}}</span>
      </div>
      ...
      <div>
        ...
        >
          <el-table-column
            ...
            width="40"
            align="center"
            v-if="currItem && !currItem.isAdded"
          ...
          <el-table-column prop="count" label="当前库存" width="70">
            <template slot-scope="scope">
              ...
              >{{scope.row.store?scope.row.store.count:0}}</span>
            </template>
          </el-table-column>
      ...
      <div class="btns" v-if="crmOrderList.length">
        ...
        <div>
          <el-pagination
            ...
            :current-page.sync="queryIn.page"
            :page-sizes="[3,20, 50, 100, 200]"
            :page-size="queryIn.pagesize"
            layout="total,sizes,prev,pager,next"
            :total="queryInTotal"
          />
          <el-button
            v-if="!currItem.isAdded"
            type="success"
            @click="inOrder"
            icon="el-icon-document"
            :loading="inLoading"
          >{{searchForm.typeId==1?'采购':'生产'}}订单入库</el-button>
