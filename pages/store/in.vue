  ...
  <section class="order-main">
    ...
    <el-dialog title="仓库入库清单" append-to-body :visible.sync="openOrderVisible" width="85%">
      ...
      <div class="btns" v-if="crmOrderList.length">
				<div>
					<span>合计：共{{queryInTotal}}个采购订单；</span><span v-if="!currItem.isAdded">请勾选需要入库的订单；已选个{{selectOrders.length}}订单</span><span v-else>订单已全部入库</span>
				</div>
				<div>
...
export default {
  ...
  methods:{
    handleSelectionChange(val){
      this.selectOrders = val.filter(item=>{return item.id});
    },
    checkSelectable(row){
    ...
    inOrder(){
      ...
      let condition = {
        ...
        set:{$set:{'isAdded':true,'updateByUser':this.$store.state.user.name,'updateDate':new Date().getTime()}}
      }
