    ...
    <div class="form-container">
      <div class="upload-container">
        ...
        >
          <el-form-item label="订单类型：">
            <el-select
              v-model="tableForm.typeId"
              placeholder="请选择"
              clearable
              @change="filterByForm"
            >
              <el-option
                v-for="types in setting.type"
                :key="types.id"
                :label="types.name"
                :value="types.id"
              />
          ...
          <el-form-item>
            ...
            <el-button
              ...
              :loading="uploading"
              icon="my-icon-save"
            >导入全部订单({{sourceData.length}})</el-button>
            <el-button type @click="sourceData=[]" icon="el-icon-refresh">重新上传</el-button>
            <el-button
              type
              @click="removeRepeat"
              v-if="repeatCount"
            >删除重复梯号、项目号的订单(共：{{repeatCount}})</el-button>
            <el-button
              type
              @click="openDialogNotin=true"
              v-if="notInList.length"
              :disabled="openDialogNotin"
            >显示无法匹配元数据的订单(共：{{notInList.length}})</el-button>
          </el-form-item>
        </el-form>
      ...
      <div v-if="sourceData.length">
        ...
        >
          ...
          <!--订单数据列表-->
          <el-table-column
            v-for="item of tableKeys.filter(o=>o.show)"
            :prop="item.value"
            :label="item.label"
            :key="item.value"
            :width="item.width"
        ...
        <div class="page-container" style="padding: 10px 0;">
          ...
          <el-pagination
            size="mini"
            @size-change="uploadSizeChange"
            @current-change="uploadCurrentChange"
            :current-page.sync="queryUpload.page"
            :page-sizes="[20, 50, 100, 200, 500]"
            :page-size="queryUpload.pagesize"
      ...
      lists.forEach(item => {
        for (let k in item) {
          if (
            ...
            item[k].includes(this.tableForm.keyword)
          ) {
        ...
        for (let k in item) {
          ...
          if (head) {
            ...
            if (head.value == "productName") {
              if (
                value.includes("轿顶防护栏") ||
                value.includes("线槽") ||
                value.includes("挂钩") ||
                value.includes("救援装置柜")
              ) {
                obj.typeId = 2;
                obj.flowStateId = 5;
              } else {
