<template>
    <section class="order-main">
				...
				<el-table-column label="操作" fixed="right" align="center" width="120">
					<template slot-scope="scope">
						<div v-if="!scope.row.storeTypeId">
							<el-button size="mini" type="text" @click="showDetail(scope.row)" icon="el-icon-edit">编辑</el-button>
        ...
        <el-dialog :title="'供应商采购清单--'+(needSource?'蒂森采购清单':'珏合采购清单')" append-to-body :visible.sync="openOrderVisible" width="80%">
            ...
            <div>
                <el-table ref="exportTable" :data="crmOrderList" border fit highlight-current-row stripe size="mini" max-height="350" v-loading="searchLoading" style="width:100%">
                    ...
                    <el-table-column prop="serial" label="系统订单号" width="120px"/>
                    <el-table-column v-if="needSource" prop="sourceserial" label="蒂森订单编号" width="200px"/>
					...
					<el-table-column prop="price" label="采购单价" width="100px">
                        <template slot-scope="scope">
                            <div v-if="scope.$index<crmOrderList.length-1">
                                <el-input-number size="mini" v-if="!scope.storeTypeId" controls-position="right" :min="0" :step="0.1" v-model="scope.row.price" @change="setRowData(scope.row)" style="width:80px"/>
							    <span v-else>{{scope.row.price}}</span>
                            </div>
						</template>
                    </el-table-column>
					<el-table-column prop="count" label="订单数量" width="80px">
                        <template slot-scope="scope">
							<span>{{scope.row.count}} {{scope.row.util}}</span>
						</template>
                    </el-table-column>
                    <el-table-column prop="incount" label="实际采购量" width="100px">
                        <template slot-scope="scope">
							<span  v-if="scope.$index<crmOrderList.length-1">
					...
					<el-table-column prop="deliveryDate" label="交付日期" width="160px">
						<template slot-scope="scope">
                            <div v-if="scope.$index<crmOrderList.length-1">
                                <el-date-picker size="mini" v-if="!scope.storeTypeId" v-model="scope.row.deliveryDate"  type="date" value-format="timestamp" placeholder="选择日期" 
            ...
            <div class="btns" v-if="crmOrderList.length">
				...
				<div>
					<el-button type="success" @click="exportOrder" icon="el-icon-document">重新制单采购</el-button>
