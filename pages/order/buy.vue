<template>
  <section class="order-main">
        ...
        >
          ...
          <el-form-item label="供应商：" prop="crmId">
            <el-select v-model="searchForm.crmId" placeholder="请选择" clearable>
              <el-option
                v-for="crm in crmList.filter(o=>o.typeId==1)"
                :key="crm.id"
                :label="crm.name"
                :value="crm.id"
              />
        ...
        <el-table-column :label="needSource?'蒂森订单号':'珏合订单号'" width="150">
          <template slot-scope="scope">
            <a
              ...
              @click="showSourceOrder(scope.row)"
            >{{scope.row.sourceserial}}</a>
    ...
    <div class="form-container" v-else>
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" size="mini">
        ...
        <el-form-item label="产品名称" prop="productId">
          ...
          >
            <el-option
              v-for="product in proList"
              :key="product.id"
              :label="product.name+'('+product.materialNo+')'"
              :value="product.id"
            />
          </el-select>
        </el-form-item>
    ...
    <el-dialog
      :title="'供应商采购清单--'+(needSource?'蒂森采购清单':'珏合采购清单')"
      append-to-body
    ...
    >
      ...
      <div class="compare" id="printTable" ref="printTable">
        <div class="order-title">
          <span
            v-if="currItem"
          >供应商：{{currItem.crmName}}，地址：{{currItem.address}}，联系人：{{currItem.contactName}}，电话：{{currItem.contactPhone}}</span>
          <span v-else>请选择供应商，查询所分发的采购订单</span>
        </div>
        <div class="order-title" style="padding:10px 0" v-if="crmOrderList.length">
          <span
            style="font-weight:bold"
          >订单总价：{{crmOrderList[crmOrderList.length-1]['allPrice'] | currency}}</span>
        </div>
        <el-table
          ref="exportTable"
          :data="crmOrderList"
        ...
        >
            ...
            <template slot-scope="scope">
              <div v-if="scope.$index<crmOrderList.length-1">
                <el-input-number
                  ...
                  :min="0"
                  v-model="scope.row.releaseCount"
                  @change="checkReleaseCount(scope.row,crmOrderList)"
                  style="width:80px"
                />
              </div>
          ...
          <el-table-column prop="deliveryDate" label="交货日期" width="140px">
            <template slot-scope="scope">
              <div v-if="scope.$index<crmOrderList.length-1">
                <el-date-picker
                  size="mini"
          ...
          <el-table-column label="操作" fixed="right" align="center" width="70">
            <template slot-scope="scope">
              <el-button
                v-if="scope.$index<crmOrderList.length-1"
                size="mini"
                type="text"
                @click="removeCrmOrder(scope.row)"
                icon="el-icon-delete"
              >移除</el-button>
            </template>
          </el-table-column>
    ...
    <el-dialog
      :title="'蒂森订单：'+sourceserial+'；请点击供应商制定或查阅采购订单'"
      append-to-body
    ...
    >
      ...
      >
        ...
        <el-table-column
          ...
          align="center"
          width="120"
          v-if="detailItem && detailItem.flowStateId==1"
        >
          <template slot-scope="scope">
            <span>
              <el-button
        ...
        <div>
          <el-form size="mini" :inline="true" :model="updateForm" :rules="updateRules">
            <el-form-item label="修改交付日期：" prop="deliveryDate">
              <el-date-picker
                v-model="updateForm.deliveryDate"
                value-format="timestamp"
                type="date"
                placeholder="选择日期"
                style="width:130px"
  ...
  methods: {
    ...
    async listOrderByCrm(match = {}) {
      ...
      let params = {
        ...
        aggregate: [
          ...
          //{$match:_.merge({"flowStateId":1,"store":{"$size":1},"store.storeTypeId":{"$gt":0}}, match)},
          /* {
    ...
    mergeOrder(lists) {
      ...
      lists.forEach(item => {
        ...
        item.children = [];
        item.storeInCount = item.store ? item.store.count : 0; // 默认库存 0
