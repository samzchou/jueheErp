<template>
    <section>
        <div class="head-title">
            <div>
                <div>
                    <span :class="{'inactive':!isPayed}" @click="setOrderParams(false)">待付款订单</span>
                    <span :class="{'inactive':isPayed}" @click="setOrderParams(true)">已付款订单</span>
                </div>
            </div>
        </div>
        <div class="grid-container" v-if="!isEdit">
                ...
                <el-form :inline="true" :model="searchForm" ref="searchForm" size="mini" @keyup.native.enter="submitSearch">
                    ...
                    <el-form-item label="供应商：" prop="crmId" v-if="setting">
                        <el-select v-model="searchForm.crmId" placeholder="请选择" clearable filterable style="width:200px">
                            <el-option v-for="item in setting.crm.filter(o=>{return o.typeId==1})" :key="item.id" :label="item.name" :value="item.id"/>
                        </el-select>
                    </el-form-item>
            ...
            <el-table v-loading="listLoading" :data="gridList" border fit highlight-current-row stripe size="mini" max-height="500">
                ...
                <el-table-column prop="orderSerial" label="制单单号" width="150"/>
                ...
                <el-table-column prop="payTotalPrice" label="采购总额" width="120">
                    <template slot-scope="scope">
                ...
                <el-table-column label="付款" fixed="right" align="center" width="70">
                    <template slot-scope="scope">
                        <div>
                            <el-button size="mini" type="text" @click="showDetail(scope.row)" icon="my-icon-retweet">{{scope.row.isPayed?'查阅':'付款'}}</el-button>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog title="订单明细查阅" :visible.sync="openDialogVisible" width="80%">
            ...
            <div>
                <el-table ref="exportTable" :data="orderCrmList" border fit highlight-current-row stripe size="mini" max-height="350" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column prop="totalPrice" label="采购金额" width="100">
                        <template slot-scope="scope">
                    ...
                    <el-table-column prop="finishedDate" label="制单交货日期" width="110">
                        <template slot-scope="scope">
            ...
            <div class="btns" v-if="orderCrmList.length">
                <div>
                    ...
                    <span v-if="currItem.isPayed">订单已付款</span>
    ...
    methods:{
        ...
        exportOrder(){
            ...
            import("@/components/Export2Excel").then(excel => {
                const tHeader = ["制单号","物料号","物料名称","规格型号","数量","订单单价","采购单价","订单金额","采购金额","订单毛利","制单交货日期"];
                const filterVal = ["orderSerial","materialNo","productName","model","incount","price","metaprice","orderPrice","totalPrice","fitPrice","finishedDate"];
                const data = this.formatJson(filterVal, _.cloneDeep(this.orderCrmList));
                const now = moment(new Date()).format("YYYYMMDD");
                excel.export_json_to_excel({
                    header: tHeader,
                    data,
                    filename: this.currItem.crmName + "-付款单-" + now
                });
            });
        },
        ...
        async showDetail(row){
            ...
            this.currItem.totalPrice = totalPrice;
        ...
        parseFitMoney(row){
            let fit = row.incount*row.price - row.incount*row.metaprice;
             return this.$options.filters['currency'](fit);
        },
        ...
        async getList(match={}){
            ...
            let param = _.merge( {typeId:1, isAdded:true, isPayed:this.isPayed}, match);
            let condition = {
                type: "groupList",
                collectionName: "storeIn",
                data: param,
